...
├── README.md
├── core/                             # 포매팅 파이프라인 핵심 모듈
│   ├── tokenizer.ts                  # SqlTokenizer — SQL 문자열 → 토큰 스트림
│   ├── parser.ts                     # SqlParser — 토큰 스트림 → AST
│   ├── formatter.ts                  # SqlFormatter — AST → 포매팅된 SQL
│   ├── autoIndentFormatter.ts        # Auto 들여쓰기 후처리 엔진
│   └── utils/                        # 공유 유틸리티
│       ├── subqueryUtils.ts          # 서브쿼리 추출·복원·들여쓰기
│       ├── mybatisUtils.ts           # MyBatis 태그/파라미터 치환·복원
│       ├── renderUtils.ts            # 토큰 렌더링·들여쓰기·케이스 변환
│       └── dialectUtils.ts           # SQL 키워드 목록·방언 감지
└── types/
    ├── config.ts                     # FormatterConfig, FormatOptions, SqlDialect
    └── token.ts                      # SqlToken, TokenType, TokenPattern
```
...
```
formatter.ts           ──→  utils/mybatisUtils.ts
                       ──→  utils/renderUtils.ts
                       ──→  utils/subqueryUtils.ts
                       ──→  utils/dialectUtils.ts

tokenizer.ts           ──→  utils/dialectUtils.ts   (SQL_KEYWORDS)

autoIndentFormatter.ts ──→  utils/subqueryUtils.ts
```
...
키워드 판별은 `utils/dialectUtils.ts`의 `SQL_KEYWORDS`를 참조한다.
...
**주요 기능**
- 키워드 식별 (`SQL_KEYWORDS` 기준, 방언 공통 + 확장)
...
렌더링·들여쓰기·케이스 변환은 `utils/renderUtils.ts`에 위임하고,
MyBatis 처리는 `utils/mybatisUtils.ts`에 위임한다.
...
| `restoreSubqueries(sql, blocks, cfg, formatInner?)` | 플레이스홀더를 포매팅된 서브쿼리로 복원 |
...
**alias 오탐 방지 규칙** — 서브쿼리 바로 뒤 토큰을 alias로 인정하지 않는 조건:
1. 비어있음
...
MyBatis XML 태그와 파라미터를 플레이스홀더로 치환·복원하는 처리기.
`formatter.ts`에서 분리.

| export | 설명 |
|--------|------|
| `MyBatisTemplateHandler` | `extract()` / `restore()` — 치환·복원 클래스 |
| `hasMybatisMarkers(sql)` | MyBatis 구문 포함 여부 감지 |
| `STRUCTURAL_TAG_RE` | `<where>`, `<set>` 구조 태그 정규식 |
| `BLOCK_TAG_RE` | `<if>`, `<foreach>` 등 블록 태그 정규식 |

**처리 흐름**
- `<![CDATA[...]]>` → open/close를 각각 별도 플레이스홀더로 보존
- `#{param}`, `${param}` → 인라인 파라미터 플레이스홀더
- `<where>` / `<set>` → sentinel 키워드 삽입으로 파서가 WHERE/SET 절을 올바르게 인식

### renderUtils.ts

토큰 배열을 SQL 문자열로 변환하는 순수 함수 모음.
`SqlFormatter` 클래스 상태에 의존하지 않아 다른 포매터 모듈에서도 재사용 가능.
`formatter.ts`에서 분리.

| export | 설명 |
|--------|------|
| `renderTokens(tokens, cfg)` | 토큰 배열 → SQL 문자열 (공백·연산자·괄호 처리 포함) |
| `applyTokenCase(token, cfg)` | 토큰에 keywordCase 설정 적용 |
| `applyKeywordCase(keyword, cfg)` | 키워드 문자열에 keywordCase 설정 적용 |
| `buildIndent(cfg, level)` | 들여쓰기 문자열 생성 (spaces / tabs) |
| `formatColumnStr(colStr, cfg, indentLevel, index, total)` | 콤마 위치(leading / trailing) 적용 |

### dialectUtils.ts

SQL 키워드 데이터와 방언 감지 로직.
`tokenizer.ts`(keywords Set) + `formatter.ts`(detectDialect)에서 분리.

| export | 설명 |
|--------|------|
| `SQL_KEYWORDS` | 방언 공통 + 확장 키워드 Set — tokenizer의 keyword/identifier 판별 기준 |
| `detectDialect(sql)` | SQL 문자열 분석으로 방언 추론 (mybatis → plsql → mysql → postgresql → transactsql → sql) |
...
- [x] 비-auto 서브쿼리 포매팅 — `autoIndent: false` 시에도 들여쓰기 정상 적용
- [x] SQL 함수 alias 오탐 방지 — DECODE, NVL, COALESCE 등 함수명을 alias로 오인하지 않음
- [x] SQL 방언 자동 감지 및 유효성 검사
- [x] 유틸리티 모듈 분리 (`core/utils/`) — mybatisUtils, renderUtils, dialectUtils, subqueryUtils
