...
- **주요 내용**
  - API Key 입력창 및 유효성 검사 (OpenAI / Anthropic 선택, 키는 LocalStorage 저장)
  - "AI 도움받기" 버튼: 현재 Input + 포맷 규칙을 LLM에 전달해 최적화된 SQL 수신
  - AI 결과를 "변경 전/후" 미리보기 모달로 표시 후 Output에 적용 또는 취소
...
│   │   └── ai.ts             # AI 프로바이더 타입 및 키 검사
│   ├── utils/
│   │   ├── formatSql.ts       # 규칙 적용 포매팅 + 콤마 위치 후처리
│   │   ├── applyReplaceRules.ts # 텍스트/정규식 대체 적용
│   │   ├── aiFormat.ts        # OpenAI/Anthropic API 호출 및 프롬프트
│   │   └── storage.ts         # LocalStorage (규칙·API Key 등)
│   ├── App.tsx                # 메인 앱, 포맷·대체·AI·저장 연동
│   ├── main.tsx
│   └── index.css
├── doc/
│   ├── setup/                 # 단계별 구현 요약
